...
DROP POLICY IF EXISTS "Public read access for applicant documents" ON storage.objects;
DROP POLICY IF EXISTS "Allow uploads for applicant documents" ON storage.objects;
DROP POLICY IF EXISTS "Allow deletes for applicant documents" ON storage.objects;

-- Create storage policies for applicant-documents bucket
CREATE POLICY "Public read access for applicant documents"
ON storage.objects FOR SELECT
USING (bucket_id = 'applicant-documents');

CREATE POLICY "Allow uploads for applicant documents"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'applicant-documents');

CREATE POLICY "Allow deletes for applicant documents"
