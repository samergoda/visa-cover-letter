...
function appendSection(sections: string[], title: string, fields: (string | null | false)[]): void {
...
export function buildCoverLetterPrompt(client: ClientInformation, tone?: string): string {
  ...
  appendSection(sections, "CONTACT & RESIDENCE:", [
    ...
    formatValue(client.currentAddress) && `Current Address: ${client.currentAddress}`,
    formatValue(client.cityOfResidence) && `City of Residence: ${client.cityOfResidence}`,
  ...
  appendSection(sections, "TRAVEL:", [
    ...
    formatValue(client.embassyCity) && `Embassy/Consulate: ${client.embassyCity}`,
    formatValue(client.numberOfEntries) && `Number of Entries: ${client.numberOfEntries}`,
  ...
  appendSection(sections, "EMPLOYMENT:", [
    ...
    formatValue(client.employerAddress) && `Employer Address: ${client.employerAddress}`,
    formatValue(client.employmentType) && `Employment Type: ${client.employmentType}`,
    formatValue(client.monthlySalary) && `Monthly Salary: ${client.monthlySalary}`,
    formatValue(client.annualIncome) && `Annual Income: ${client.annualIncome}`,
  ...
  appendSection(sections, "FINANCIAL:", [
    ...
    formatValue(client.tripFundedBy) && `Trip Funded By: ${client.tripFundedBy}`,
  ...
  appendSection(sections, "TRAVEL HISTORY:", [
    ...
    formatValue(client.countriesVisited) && `Countries Visited: ${client.countriesVisited}`,
    formatValue(client.schengenVisasHeld) && `Schengen Visas Held: ${client.schengenVisasHeld}`,
  ...
  appendSection(sections, "RESERVATIONS & SPONSORSHIP:", [
    ...
    formatValue(client.sponsorInformation) && `Sponsor Information: ${client.sponsorInformation}`,
  ]);

  appendSection(sections, "HOME COUNTRY TIES:", [
    formatValue(client.familyTiesHomeCountry) && `Family Ties: ${client.familyTiesHomeCountry}`,
    formatValue(client.reasonToReturn) && `Reason to Return: ${client.reasonToReturn}`,
  ]);
